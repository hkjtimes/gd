...
> **ВНИМАНИЕ:** раздел ниже описывает только три режима и устарел.
> Актуальный контракт по режимам — §5b в конце файла: восемь режимов
> передвижения и три модификатора. При расхождении приоритет у §5b.
...
# §5b — ВСЕ игровые режимы Geometry Dash

Дополнение к `docs/SPEC.md`. Заменяет всё, что там сказано про режимы.
Раньше было три режима (cube/ship/wave) — этого мало: в GD их восемь,
плюс три модификатора. Агент обязан уметь всё.

---

## 1. Восемь режимов передвижения

`PlayerState.mode` ∈ `("cube","ship","ball","ufo","wave","robot","spider","swing")`.

Управление везде одно и то же: одна кнопка. Отличается только реакция на неё.
`hold` — кнопка нажата сейчас; `tap = hold and not hold_prev` — фронт нажатия.

| Режим | Реакция на кнопку | Ключевая механика |
|---|---|---|
| `cube` | `hold` на земле | Прыжок `vy = JUMP_V`, высота ≈2.4 тайла. При зажатой кнопке прыгает повторно сразу после приземления. |
| `ship` | `hold` = тяга | `vy += SHIP_THRUST*dt` при удержании, `vy -= SHIP_GRAVITY*dt` иначе. Клип `±SHIP_MAX_V`. Может скользить по полу и потолку, боковой удар — смерть. |
| `ball` | `tap` на поверхности | Мгновенный переворот гравитации (`gravity *= -1`), игрок «падает» к противоположной поверхности. В воздухе тап игнорируется. |
| `ufo` | `tap` где угодно | `vy = UFO_JUMP_V` даже в воздухе — многократный прыжок. Обычная гравитация. |
| `wave` | `hold` | Мгновенная скорость `vy = ±speed*WAVE_RATIO`, без инерции, движение под 45°. Любое касание твёрдого — смерть. |
| `robot` | `hold` с зарядкой | На земле `tap` начинает прыжок; пока кнопка удерживается (до `ROBOT_HOLD_FRAMES`), к скорости добавляется тяга. Отпустил раньше — прыжок ниже. Высота от ≈1.5 до ≈3.4 тайла. |
| `spider` | `tap` | Мгновенная **телепортация** к ближайшей твёрдой поверхности в противоположном направлении + переворот гравитации. `vy = 0`. Если поверхности нет — телепорт не выполняется. |
| `swing` | `hold` | Пока кнопка нажата — гравитация вверх, отпущена — вниз (`gravity_effective = -gravity if hold else gravity`). Получается плавная дуга. |

Новые константы в `gdai/constants.py`:
...
UFO_JUMP_V        = 15.0
BALL_GRAVITY      = 76.8      # как у куба
BALL_MAX_V        = 22.0
ROBOT_JUMP_V      = 13.5      # стартовый импульс
ROBOT_HOLD_ACCEL  = 96.0      # доп. тяга во время удержания
ROBOT_HOLD_FRAMES = 18        # максимум ~0.3 с зарядки
SPIDER_MAX_TELEPORT = 12.0    # максимальная дальность телепорта, тайлы
SWING_GRAVITY     = 62.0
SWING_MAX_V       = 18.0
WAVE_RATIO        = 1.0       # 45°; мини-волна круче: WAVE_RATIO_MINI = 2.0
```
...
### 2.1 Мини (`portal_mini` / `portal_big`)
`PlayerState.mini: bool`. Хитбокс × `MINI_SCALE = 0.6`. Влияет на параметры:

```python
MINI_SCALE      = 0.6
JUMP_V_MINI     = 15.4    # прыжок ниже (~1.55 тайла), но чаще
SHIP_THRUST_MINI, SHIP_GRAVITY_MINI = 62.0, 62.0   # мини-корабль резче
WAVE_RATIO_MINI = 2.0     # мини-волна идёт круче 45°
UFO_JUMP_V_MINI = 12.5
ROBOT_JUMP_V_MINI = 11.0
```

### 2.2 Дуал (`portal_dual` / `portal_single`)
Два игрока одновременно, управляются **одной** кнопкой. Второй — зеркальная копия
по вертикали (стартует с противоположной гравитацией). Смерть любого = смерть эпизода.

Реализация: `PlayerState.partner: PlayerState | None`. `step_physics` при наличии
партнёра шагает обоими и объединяет события (`died = a.died or b.died`).
Оба игрока рисуются классом `PLAYER` в семантической карте.
При `portal_single` партнёр удаляется (остаётся тот, что ближе к центру).

### 2.3 Зеркало (`portal_mirror` / `portal_normal`)
`PlayerState.mirrored: bool`. Движение по X меняет знак (игрок едет влево),
камера зеркалится. Реализация — самая аккуратная:
* физика: `x += direction * speed * dt`, где `direction = -1 if mirrored else 1`;
* `camera_origin` возвращает флаг зеркала, а `render_semantic` и `Renderer.render`
  **оба** применяют `np.fliplr` к готовому кадру. Так карта и картинка остаются
  согласованными пиксель-в-пиксель, а политика видит привычную ей картину
  «еду вправо» — это правильно и делает зеркало бесплатным для обучения.

---

## 3. Семантические классы: расширение до 13

Добавляются три класса (порядок фиксирован, старые значения не меняются):

```python
PORTAL_SIZE   = 10   # мини / обычный размер
PORTAL_DUAL   = 11   # дуал / одиночный
PORTAL_MIRROR = 12   # зеркало / нормальный ход
NUM_CLASSES   = 13
```

`CLASS_NAMES` и `CLASS_COLORS` расширить соответственно. Цвета новых классов
должны хорошо различаться на глаз от уже существующих.

---

## 4. Типы объектов уровня: расширение

К `OBJECT_TYPES` добавить:

```
"portal_ball", "portal_ufo", "portal_robot", "portal_spider", "portal_swing"   -> PORTAL_MODE
"portal_mini", "portal_big"                                                    -> PORTAL_SIZE
"portal_dual", "portal_single"                                                 -> PORTAL_DUAL
"portal_mirror", "portal_normal"                                               -> PORTAL_MIRROR
```

(`portal_cube`, `portal_ship`, `portal_wave` уже есть.)

---

## 5. Вектор признаков: `FEATURE_DIM = 16`
...
[6]      dual
[7]      mirrored
[8..15]  one-hot режима по MODES (8 значений)
```

---

## 6. Что обязано обновиться вместе с этим

* `constants.py` — все константы выше, `NUM_CLASSES = 13`, `FEATURE_DIM = 16`.
* `env/physics.py` — восемь режимов, три модификатора, дуал-партнёр.
* `env/level.py` — новые типы объектов и их `semantic_class` / `half_extent`.
* `env/semantic.py` — отрисовка новых порталов (различимые ПО ФОРМЕ иконки:
  у каждого режима свой глиф внутри овала портала), поддержка зеркала и дуала.
* `env/generator.py` — паттерны под каждый режим: ball-коридор, ufo-пещера,
  robot-платформы с разной высотой прыжка, spider-шахта, swing-дуга, mini-секции,
  дуал-секция, зеркальная секция. `is_solvable` обязан их проходить.
* `env/render.py`, `env/themes.py` — визуальные стили новых порталов и игрока
  в каждом режиме (форма игрока отличается: куб, треугольник-корабль, круг-шар,
  тарелка-НЛО, стрелка-волна, прямоугольник-робот, «паук», «качели»).
* `agent/networks.py` — `FEATURE_DIM = 16` и `NUM_CLASSES = 13` берутся из констант.
* `perception/*` — число классов из констант, веса классов для новых порталов.
* `tests/` — `test_physics_modes.py`: по тесту на каждый режим и модификатор;
  проходимость сгенерированных уровней с каждым режимом.
* `README.md`, `docs/ARCHITECTURE.md` — таблица режимов.

## 7. Критерий приёмки

1. Для каждого из 8 режимов и каждого из 3 модификаторов есть тест, проверяющий
   ключевую механику числом (высота прыжка, факт телепорта, переворот гравитации…).
2. Генератор умеет строить уровень, где встречаются **все** режимы, и `is_solvable`
   подтверждает его проходимость.
3. `python -m gdai play` позволяет пройти такой уровень руками.
4. Обученная политика показывает ненулевой success_rate на смешанных уровнях.
